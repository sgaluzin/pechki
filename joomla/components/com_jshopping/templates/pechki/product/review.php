...
<?php if ($this->allow_review){?>
    <div class="review">
        <? $count = count($this->reviews); if ($count>0) print '<h2 class="review-header">Отзывы: '.$count.'</h2>';?>
        <?php foreach($this->reviews as $curr){?>
            <div class="review-item">
                <div><span class="review-user"><?php print $curr->user_name?></span>, <span class='review-time'><?php print formatdate($curr->time);?></span></div>
                <div class="review-text"><?php print nl2br($curr->review)?></div>
                <?php if ($curr->mark) {?>
                    <div class="review-mark"><?php print showMarkStar($curr->mark);?></div>
                <?php } ?>
            </div>
        <?php }?>
        ...
        <?php if ($this->allow_review > 0){?>
            ...
            <h3 class="review"><?php print _JSHOP_ADD_REVIEW_PRODUCT?></h3>
